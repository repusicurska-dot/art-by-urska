...
const HREFS = ["/collection", "/poetry", "/contact"] as const;
...
export default function FinalInvitation() {
  ...
  const { t } = useLanguage();
  const OPTIONS = [
    { label: t.home.exploreWorks, href: HREFS[0] },
    { label: t.home.discoverPoetry, href: HREFS[1] },
    { label: t.home.commission, href: HREFS[2] },
  ];

  return (
    <section className="relative h-screen min-h-[640px] w-full overflow-hidden">
      <div className="relative hidden h-full flex-col items-center justify-center px-6 text-center md:flex">
        <h2 className="font-gothic text-4xl text-bone md:text-6xl">{t.home.finalTitle}</h2>
      ...
      <div className="flex h-full flex-col items-center justify-center gap-8 px-6 text-center md:hidden">
        <h2 className="font-gothic text-3xl text-bone">{t.home.finalTitle}</h2>
        {OPTIONS.map((o) => (
          <Link
            key={o.href}
            href={o.href}
