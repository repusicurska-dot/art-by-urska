...
export default function LogoTransition({ epigraph }: { epigraph?: string }) {
  return (
    <motion.div
      className="fixed inset-0 z-[100] pointer-events-none flex flex-col items-center justify-center gap-6 px-10 text-center"
      style={{ background: "var(--color-ink)" }}
      aria-hidden="true"
      initial={{ opacity: 1 }}
      animate={{ opacity: 0 }}
      transition={{ duration: 0.6, ease: [0.65, 0, 0.35, 1], delay: 1.1 }}
    >
      <ArrivalLogo />
      {epigraph && (
        <motion.p
